...
CFractalApp::CFractalApp()
{
	...
	DebugMsg(_T("the Free Software Foundation; version 3 of the License.\n"));
	DebugMsg(_T("The MIT license does not apply to this work"));
	DebugMsg(_T("\n"));
...
BOOL CFractalApp::InitInstance()
{
	...
	bool bRegServer = cmdInfo.m_nShellCommand == CCommandLineInfo::AppRegister;
	cmdInfo.m_nShellCommand = CCommandLineInfo::FileNothing;

	// Dispatch commands specified on the command line.  Will return FALSE if
	// app was launched with /RegServer, /Register, /Unregserver or /Unregister.
	if (bRegServer || !ProcessShellCommand(cmdInfo))
		return FALSE;
